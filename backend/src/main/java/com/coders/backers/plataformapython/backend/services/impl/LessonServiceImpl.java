...
public class LessonServiceImpl implements LessonService {
    ...
    public LessonDto createLesson(CreateLessonDto createLessonDto) {
        CourseEntity courseEntity = courseRepository.findById(createLessonDto.getCourseId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Curso no encontrado con id: " + createLessonDto.getCourseId()));
    ...
    public List<LessonDto> getActiveLessonsByCourseIdAndLevel(Long courseId, String level) {
        ...
        List<LessonEntity> activeLessons = lessonRepository.findByCourseIdAndCourseLevelAndActive(courseId, level,
                true);
        return activeLessons.stream()
                .map(LessonMapper::mapToModelDto)
                .collect(Collectors.toList());
    ...
    public LessonDto updateLesson(Long id, UpdateLessonDto updateLessonDto) {
        ...
        // cuando sea necesario. Por ahora, las relaciones se manejan a través de
        // ContenidoModel

        // Actualizar el curso si ha cambiado
        if (updateLessonDto.getCourseId() != null &&
                (lessonEntity.getCourse() == null
                        || !updateLessonDto.getCourseId().equals(lessonEntity.getCourse().getId()))) {
            CourseEntity courseEntity = courseRepository.findById(updateLessonDto.getCourseId())
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Curso no encontrado con id: " + updateLessonDto.getCourseId()));
    ...
    public void deleteLesson(Long id) {
        ...
        try {
            ...
            throw new IllegalStateException("No se puede eliminar la lección porque tiene dependencias activas", e);
        }
    }
