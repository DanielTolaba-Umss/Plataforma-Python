...
const [output, setOutput] = useState("");
  ...
  setPracticeData({
    ...
    leccionId: practica.leccionId, // Corregido aquí

    id: practica.id, // Asegurar que incluimos el id para edición
 ...
 useEffect(() => {
  const fetchPracticas = async () => {
    try {
      ...
      const filtradas = response.data.filter(p => Number(p.leccionId) === Number(courseId));
...
const handleSubmitTestCase = async (e) => {
  ...
  const formattedInput = input.startsWith("IN: ") ? input : `IN: ${input}`;
  const formattedOutput = output.startsWith("OUT: ") ? output : `OUT: ${output}`;

  const testCaseData = {
  entrada: `"${formattedInput.replaceAll('"', '\\"')}"`,
  salida: `"${formattedOutput.replaceAll('"', '\\"')}"`,
  practiceId: practiceData.id,
};


  console.log("Enviando test case:", testCaseData);

  try {
    // Aquí decides si es un test case nuevo o existente
    if (currentTestCase.id) {
      await testCasesAPI.actualizar(currentTestCase.id, testCaseData);
    } else {
      await testCasesAPI.crear(testCaseData);
    }

    showNotification("✅ Test case enviado correctamente.");

    // Actualiza la lista de test cases locales
    setPracticeData(prev => ({
      ...prev,
      testCases: [...(prev.testCases || []), testCaseData],
    }));
    ...
    showError("❌ Error al enviar el test case");
  }
};
...
function formatTestCaseString(str, prefix) {
  if (!str || str.length < prefix.length + 1) {
    // si está vacío o muy corto, por ejemplo "" o "IN:" sin nada, agregamos texto por defecto
...
function fixInput(str) {
  const prefix = "IN: ";
  if (!str || str.length < prefix.length + 1) {
    return prefix + "default";  // ejemplo valor seguro
  }
  if (!str.startsWith(prefix)) {
    return prefix + str;
  }
...
const payload = {
  entrada: fixInput("dd"),  // "IN: dd"
  salida: fixOutput("dd"),  // "OUT: dd"
  practiceId: 24
};

// Usar así:
const formattedInput = formatTestCaseString(input, "IN: ");
const formattedOutput = formatTestCaseString(output, "OUT: ");
 ...
 const handleSubmitPractice = async (e) => {
  ...
  try {
    ...
    if (modalMode === "editar") {
      response = await practiceAPI.actualizar(practiceData.id, practicePayload);
    } else {
  ...
  practiceId: practiceId,
    ...
    showNotification(`✅ Práctica ${modalMode === "editar" ? "actualizada" : "creada"} correctamente.`);
    resetPracticeForm();

    const refrescar = await practiceAPI.obtenerTodas();
    const filtradas = refrescar.data.filter(p => Number(p.leccionId) === Number(courseId));
    setPracticas(filtradas);
...
const addTestCase = () => {
  ...
  // Reinicia el formulario siempre
  setCurrentTestCase({
    entrada: "",
    salida: "",
    //entradaTestCase: "",
...
{showDeleteModal && (
  <div className="recursos-modal-overlay">
    <div className="recursos-modal-content">
      ...
      <p>
        ...
        <strong>
          {practicaAEliminar?.instrucciones?.slice(0, 40) || "Práctica sin título"}
          ...
        </strong>
        ?
      </p>
              ...
              <div key={index} className="test-case-item">
                <div>
                  ...
                  {tc.entradaTestCase && (
                    <>
                      <br />
                      <strong>Entrada para test:</strong> {tc.entradaTestCase}
                    </>
                  )}
                </div>
                <button
  ...
  <button
    ...
    disabled={!currentTestCase.entrada || !currentTestCase.salida}
  >
